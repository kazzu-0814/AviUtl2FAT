...
& $dotnet publish (Join-Path $root 'src\AviUtl2FAT.App\AviUtl2FAT.App.csproj') -c Release -r win-x64 --self-contained true --no-restore -p:BuildProjectReferences=false -p:PublishSingleFile=false -p:DebugType=None -p:DebugSymbols=false -o $app
...
foreach ($document in @('README.md', 'QUICKSTART.md', 'LICENSE', 'THIRD_PARTY_NOTICES.txt', 'RELEASE_CHECKLIST.md')) {
    Copy-Item -LiteralPath (Join-Path $root $document) -Destination $dist -Force
}
$required = @('AviUtl2FAT.aux2', 'FAT\AviUtl2FAT.App.exe', 'FAT\AviUtl2FAT.Worker.exe', 'FAT\runtime\python\fat_worker.py', 'FAT\runtime\ffmpeg\ffmpeg.exe', 'FAT\runtime\ffmpeg\ffprobe.exe')
