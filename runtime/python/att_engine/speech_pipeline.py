...
def _available_ram_bytes() -> int | None:
    """Return currently available RAM without making psutil a runtime requirement."""
    try:
        import psutil
...
def select_profile(profile: str, logical_cores: int | None = None, available_ram_bytes: int | None = None, cuda: bool | None = None) -> SpeechProfile:
    ...
    The low profile deliberately uses ``base``.  ``small`` is a larger, explicit
    model choice and must never be selected merely because the memory probe was
    unavailable.
    """
    ...
    # CUDA probing can block in a graphics driver call on some Windows PCs.
    # Profile selection must always be fast and must never prevent the first
    # progress event after audio conversion.  GPU use is therefore opt-in:
    # callers that have already performed a safe capability check may pass
    # cuda=True; automatic selection remains CPU-safe.
